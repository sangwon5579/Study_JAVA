package com.beyond.control;

import com.beyond.control.practice.A_If;
import com.beyond.control.practice.B_Switch;
import com.beyond.control.practice.C_For;
import com.beyond.control.practice.D_While;
import com.beyond.control.practice.E_Branch;
import com.beyond.control.practice.F_LoopPractice;

public class Application {
    public static void main(String[] args) {
        //new A_If().method1();
        //new A_If().method2();
        //new A_If().method3();
        //new A_If().practice();
        //new B_Switch().method1();
        //new B_Switch().method2();
        //new B_Switch().practice();
        //new C_For().method1();
        //new C_For().method2();
        //new C_For().method3();
        //new D_While().method1();
        //new D_While().method2();
        //new E_Branch().method1();
        //new E_Branch().method2();

        new F_LoopPractice().practice1();
        new F_LoopPractice().practice2();
        new F_LoopPractice().practice3();
        new F_LoopPractice().practice4();
        new F_LoopPractice().practice5();
        new F_LoopPractice().practice6();
    }
}
